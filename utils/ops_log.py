...
async def emit_command_error(
    ...
    return await emit_interaction_error(
        interaction,
        exc,
        title=f'Slash command failed: /{command_name}',
...
async def emit_component_error(
    interaction: discord.Interaction,
    error: Exception,
    item: discord.ui.Item,
) -> None:
    await emit_interaction_error(
        interaction,
        error,
        title='Component interaction failed',
        component=str(getattr(item, 'custom_id', item)),
    )

    message = 'ボタン操作中にエラーが発生しました。しばらくしてからもう一度お試しください。'
